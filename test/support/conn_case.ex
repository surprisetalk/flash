defmodule Flash.ConnCase do
  ...
  using do
    quote do
      ...
      import Flash.Router.Helpers

      import Flash.Factory

      # The default endpoint for testing
      @endpoint Flash.Endpoint
      ...
      def guardian_login(%Flash.User{} = user),
        do: guardian_login(build_conn(), user, :token, [])

      def guardian_login(%Flash.User{} = user, token),
        do: guardian_login(build_conn(), user, token, [])

      def guardian_login(%Flash.User{} = user, token, opts),
      ...
      def guardian_login(%Plug.Conn{} = conn, user, token, opts) do
        conn
        |> bypass_through(Flash.Router, [:browser])
        |> get("/")
